  ...
  <div>
    <div class="border rounded w-full p-1 sm:p-2 mb-0.5">
      <div class="relative w-full aspect-square">
        <div
          class="absolute rounded-full bg-gray-400 right-0"
          style={{ width: `${8 * Math.pow(10.53, 0.5)}%`, height: `${8 * Math.pow(10.53, 0.5)}%` }}
        ></div>
        <div
          class="absolute rounded-full bg-gray-400 bottom-0"
  ...
  <div>
    <div class="border rounded w-full p-1 sm:p-2 mb-0.5">
      <div class="relative w-full aspect-square flex items-center justify-center">
        <div
          class="rounded-full bg-gray-400 right-0"
  ...
  <div>
    <div class="border rounded w-full p-1 sm:p-2 mb-0.5">
      <div class="relative w-full aspect-square flex items-center justify-center">
        <div
          class="rounded-full bg-gray-400 right-0"
          style={{ width: `${8 * Math.pow(1.57, 0.5)}%`, height: `${8 * Math.pow(1.57, 0.5)}%` }}
        ></div>
      </div>
    </div>
...
<script>
  const MyComponent = (props: number = 1) => {
    const [counter, refCounter] = $.createState(props);

    return $('div', { className: 'bg-orange-300 p-2 rounded flex gap-2' }, [
      $('div',
        { ref: refCounter }, // 👈 put ref in any element you want to re-render
        ['Count: ', counter.get]),
      $('div',
        { className: 'ml-auto' },
        'No re-rendered here'),
...
<script>
  // Global state
  const [search, refSearch] = $.createState('');
  ...
  const ToDo = (props) => {
    // Local state
    const [todos, refTodos] = $.createState(props.initialToDos);
    ...
    return $('div', { className: 'flex-1 flex flex-col' }, [
      ...
      $(
        'ul',
        {
          ...
          ref: [refTodos, refSearch], // 👈 Re-render when todos or search changed
        },
        () => {
          return todos.current.map((todo) => {
            if (!todo.title.toLowerCase().includes(search.get())) return null;
            return $('li', {}, [
  ...
  $(
    ...
    [
      ...
      $('div', { className: 'flex gap-2 pt-3 items-stretch' }, [
        ToDo({
          heading: 'Foo',
          initialToDos: [{ title: 'Buy milk' }, { title: 'Buy tomato' }, { title: 'Go to work' }],
        }),
        ToDo({
          heading: 'Bar',
